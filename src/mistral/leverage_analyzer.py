...
from src.mistral.tools.eval_data import *
...
def _agent_call(
    ...
    while iteration < max_iteration:
        ...
        for tool_call in tool_calls:
            ...
            if verbose:
                print(
                    f"Tool {function_name} was called with the parameters: {function_params} and the result was: {function_result}"
                )
...
def leverage_analyzer():
    ...
    for dct in j_result["leverages_per_supplier"]:
        info_file.loc[info_file["supplier"] == dct["supplier"], "leverage"] = dct[
            "leverage"
        ]
