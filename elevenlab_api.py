...
def delete_knowledge_base(client, key, agent_id="7ZNPyYITxhYm97d1kY0q"):
    ...
    response = requests.get(
        "https://api.elevenlabs.io/v1/convai/knowledge-base", headers=headers
    )
    for dic in response.json()["documents"]:
        id = dic["id"]
        remove_knowledge_base_from_agent(client, agent_id=agent_id)
        response = requests.delete(
            f"https://api.elevenlabs.io/v1/convai/knowledge-base/{id}", headers=headers
        )
...
def link_knowledge_base_to_agent(
    client, agent_id="7ZNPyYITxhYm97d1kY0q", document_id="jZCNew80TpRKjr6K6sPZ"
):
